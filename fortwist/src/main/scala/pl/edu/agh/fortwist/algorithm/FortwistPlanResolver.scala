...
final case class FortwistPlanResolver() extends PlanResolver[FortwistConfig] {
  ...
  override def applyUpdate(iteration: Long, contents: CellContents, update: Update)(implicit
      config: FortwistConfig
  ): (CellContents, FortwistMetrics) = {
    val newContents: CellContents = (contents, update) match {
      case (
            Seabed(foraminiferas, algae),
            SeabedUpdate(
              foraminiferasToAdd,
              foraminiferasToRemove,
              foraminiferaReplacements,
              algaeDiff
            )
          ) =>
        val remainingForaminiferas = foraminiferas
          .filterNot { foraminifera =>
            foraminiferasToRemove.contains(foraminifera.id)
          }
          .map { foraminifera =>
            foraminiferaReplacements.find(_.id == foraminifera.id).getOrElse(foraminifera)
          }
