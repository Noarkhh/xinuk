...
final case class FortwistPlanCreator() extends PlanCreator[FortwistConfig] {
  override def createPlans(
    ...
    val forminPlans: Seq[(Option[Direction], Plan)] = seabed.foraminiferas.map { foraminifera =>
      if (foraminifera.energy > config.foraminiferaReproductionThreshold) {
        ...
        None -> Plan(
          ...
          SeabedUpdate(foraminiferasToChange =
            Seq(foraminifera.changed(-config.foraminiferaReproductionCost))
          )
        )
        ...
        None -> Plan(
          SeabedUpdate(
            foraminiferasToChange = Seq(foraminifera.changed(config.algaeEnergeticCapacity)),
            algaeDiff = -config.algaeEnergeticCapacity
          )
        )
        ...
        val direction: Direction = neighbourContents.keys.toSeq
          ...
          ._1
        Some(direction) -> Plan(
          SeabedUpdate(foraminiferasToAdd =
            Seq(foraminifera.changed(-config.foraminiferaLifeActivityCost))
          ),
          SeabedUpdate(foraminiferasToRemove = Seq(foraminifera.id))
        )
      }
    ...
    (
      Plans(forminPlans :+ (None -> algaeGrowthPlan)),
      FortwistMetrics(
        foraminiferaCount,
        algaeCount,
        foraminiferaDeaths,
