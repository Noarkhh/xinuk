...
object GridSignalPropagation {
  ...
  @inline private def getPropagatedSignal(
      neighbourStates: Map[Direction, CellState],
  ...
  @inline private def getGeneratedSignal(
      neighbourStates: Map[Direction, CellState],
      neighbourDirection: Direction,
      iteration: Long
  )(implicit config: XinukConfig): Signal = {
    neighbourStates
      .get(neighbourDirection)
      .map(_.contents.generateSignal(iteration))
      .getOrElse(Signal.zero)
  }

  private final object GridSignalPropagationStandard extends SignalPropagation {
    def calculateUpdate(iteration: Long, neighbourStates: Map[Direction, CellState])(implicit
        config: XinukConfig
    ): SignalMap = {
      config.worldType.directions
        .map({
          case cardinal @ (GridDirection.Top | GridDirection.Right | GridDirection.Bottom |
              GridDirection.Left) =>
            (
              cardinal,
              cardinal.withAdjacent
                .map { d => getPropagatedSignal(neighbourStates, cardinal, d) }
                .reduce(_ + _) +
                getGeneratedSignal(neighbourStates, cardinal, iteration)
            )
          case diagonal @ (GridDirection.TopLeft | GridDirection.TopRight |
              GridDirection.BottomRight | GridDirection.BottomLeft) =>
            (
              diagonal,
        ...
        .map(direction =>
          (
            ...
            getPropagatedSignal(neighbourStates, direction, direction) * direct +
              direction.adjacent
                .map { d => getPropagatedSignal(neighbourStates, direction, d) }
                .reduce(_ + _) * adjacent +
