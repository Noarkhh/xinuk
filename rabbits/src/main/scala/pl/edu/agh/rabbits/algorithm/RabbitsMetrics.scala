...
final case class RabbitsMetrics(
    rabbitCount: Long,
    lettuceCount: Long,
    rabbitDeaths: Long,
  ...
  override def +(other: Metrics): RabbitsMetrics = {
    other match {
      ...
      case RabbitsMetrics(
            ...
            otherLettuceCount,
            otherRabbitDeaths,
            otherRabbitTotalEnergy,
            otherRabbitReproductionsCount,
            otherConsumedLettuceCount,
            otherRabbitTotalLifespan,
            otherLettuceTotalLifespan
          ) =>
        RabbitsMetrics(
      ...
      case _ =>
        throw new UnsupportedOperationException(s"Cannot add non-RabbitsMetrics to RabbitsMetrics")
    }
  }
}
...
object RabbitsMetrics {
  ...
  def rabbitAddedEnergy(energy: Double): RabbitsMetrics =
    RabbitsMetrics(0, 0, 0, energy, 0, 0, 0, 0)
  def lettuce: RabbitsMetrics = Lettuce
  def rabbitDeath(rabbit: Rabbit): RabbitsMetrics =
    RabbitsMetrics(0, 0, 1, 0, 0, 0, rabbit.lifespan, 0)
  def rabbitReproduction: RabbitsMetrics = RabbitReproduction
  def lettuceConsumed(lettuce: Lettuce): RabbitsMetrics =
    RabbitsMetrics(0, 0, 0, 0, 0, 1, 0, lettuce.lifespan)
}
