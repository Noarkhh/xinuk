  ...
  override def applyUpdate(iteration: Long, contents: CellContents, update: Update)(implicit
    ...
    val (newContents: CellContents, metrics: RabbitsMetrics) = (contents, update) match {
      ...
      case (lettuce: Lettuce, CreateRabbit) =>
        ...
        (
          newRabbit,
          RabbitsMetrics.rabbit(newRabbit) + RabbitsMetrics.rabbitReproduction + RabbitsMetrics
            .lettuceConsumed(lettuce)
        )
      ...
      case (Rabbit(energy, lifespan), CreateLettuce) =>
        ...
        (
          newRabbit,
          RabbitsMetrics.rabbitAddedEnergy(config.lettuceEnergeticCapacity) + RabbitsMetrics
            .lettuceConsumed(Lettuce(0))
        )
