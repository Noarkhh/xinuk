...
final case class RabbitsPlanCreator() extends PlanCreator[RabbitsConfig] {
  ...
  private def lettucePlanning(iteration: Long, neighbourContents: Map[Direction, CellContents])(
      implicit config: RabbitsConfig
  ): (Plans, RabbitsMetrics) = {
    val ageLettucePlan = Plan(KeepLettuce)

    val spreadMap: Seq[(Option[Direction], Plan)] =
      if (iteration % config.lettuceReproductionFrequency == 0) {
        val availableDirections = neighbourContents
          .filter {
            case (_, Empty)     => true
        ...
        if (rabbit.energy > config.rabbitReproductionThreshold) {
          ...
          Plans(
            Some(direction) -> Plan(
              CreateRabbit,
              KeepRabbit(
                Rabbit(rabbit.energy - config.rabbitReproductionCost, rabbit.lifespan + 1)
              ),
              KeepRabbit(agedRabbit)
            )
          )
        } else {
          val direction: Direction = availableDirections
            .map { direction => (direction, signalMap(direction)) }
            .maxBy(_._2)
            ._1
          Plans(
            Some(direction) -> Plan(
  ...
  )(implicit config: RabbitsConfig): Direction = {
