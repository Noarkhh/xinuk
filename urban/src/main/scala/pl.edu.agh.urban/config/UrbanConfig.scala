...
final case class UrbanConfig(
  ...
  val colorToTileType: Map[Color, TileType] = tileTypes.map { tileType =>
    (tileType.color, tileType)
  }.toMap

  val targetTypeToTargets: Map[TargetType, Seq[TargetInfo]] =
    TargetType.values.map { targetType =>
      (targetType, targets.filter(_.targetTypes.contains(targetType)))
    }.toMap
  ...
  def getTimeOfDay(time: Double): Option[TimeOfDay] = {
    ...
    personBehavior.spawnRoutine
      .find { case (_, spawnProfile) =>
        currentTime.isAfter(spawnProfile.beginning) && currentTime.isBefore(spawnProfile.end)
      }
      .map(_._1)
  }

  def getPersonSpawnProbability(timeOfDay: TimeOfDay, population: Long): Double = {
    val percent = personBehavior
      .spawnRoutine(timeOfDay)
      .departurePercent // percent of population departing each 15 minutes
    (population * percent / 100d) / (900d / timeStep)
  }

  def randomSegmentDuration(): Double =
    wanderSegmentDurationMean + random.nextGaussian() * wanderSegmentDurationStd

  def randomSegments(): Long =
    wanderSegmentsMean + random.nextLong(wanderSegmentsSpread * 2 + 1) - wanderSegmentsSpread
...
object UrbanConfig {
  private val cellToColorSignal: PartialFunction[CellState, Color] = { case cellState =>
    val hue = cellState.contents match {
      case Obstacle     => 0.66f
      case _: UrbanCell => 0.0f
      case _            => 0.33f
    }
    val mag: Signal = cellState.signalMap.toSeq
      .map(_._2)
  ...
  private val cellToColorType: PartialFunction[CellState, Color] = { case cellState =>
    cellState.contents match {
      case Obstacle         => Color.BLUE
      case urban: UrbanCell =>
        urban.occupants.size match {
...
case class TimeState(
    markerRound: Long,
    time: Double,
    personMovementIteration: Boolean,
    markerPurgeIteration: Boolean
)

case class Coordinates(x: Int, y: Int) {
  def gridId(implicit config: UrbanConfig): GridCellId =
    GridCellId(x / config.zoomOut, y / config.zoomOut)
}
...
case class PersonBehavior(
    spawnRoutine: Map[TimeOfDay, HumanSpawnProfile],
    visitingRoutine: Map[TargetType, DurationDistribution],
    restrictionFactors: Map[TargetType, Double]
)

case class HumanSpawnProfile(
    beginning: LocalTime,
    end: LocalTime,
