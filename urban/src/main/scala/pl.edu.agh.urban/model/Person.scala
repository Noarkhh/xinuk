...
case class Person(
  ...
  def withAddedDecision(cellId: CellId, direction: Direction)(implicit
      config: UrbanConfig
  ): Person = {
...
object Person {
  ...
  def wandering(source: String, target: String, time: Double)(implicit
      config: UrbanConfig
  ): Person = {
    Person(
      source,
      target,
      TravelMode.Wander,
      time + config.randomSegmentDuration(),
      config.randomSegments() - 1
    )
  }
}

case class PersonMarker(
    personId: String,
    round: Long,
    distance: Double = 0d,
    sourceDirections: Set[Direction] = Set.empty
) {
  def spread(spreadDirection: Direction)(implicit config: UrbanConfig): PersonMarker = {
    val opposite = spreadDirection.opposite
    val distanceAdded =
      config.scale * (if (spreadDirection.asInstanceOf[GridDirection].isDiagonal) math.sqrt(2d)
                      else 1d)
