...
final case class UrbanPlanResolver() extends PlanResolver[UrbanConfig] {
  ...
  override def isUpdateValid(iteration: Long, contents: CellContents, update: Update)(implicit
      config: UrbanConfig
  ): Boolean = {
  ...
  override def applyUpdate(iteration: Long, contents: CellContents, update: Update)(implicit
      config: UrbanConfig
  ): (CellContents, UrbanMetrics) = {
    (contents, update) match {
      ...
      case (cell @ UrbanCell(_, entrances, _, _), RemoveVisitor(targetId, personId)) =>
        ...
        val newEntrances =
          entrances.filterNot(_.targetId == targetId) :+ modifiedEntrance.copy(visitors =
            newVisitors
          )
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"Illegal update applied: contents = $contents, update = $update"
        )
    }
  }

  private def addMarker(
      oldMarkers: Seq[PersonMarker],
      newMarker: PersonMarker
  ): Seq[PersonMarker] = {
    oldMarkers.filterNot(_.personId == newMarker.personId) :+
      oldMarkers
        .find(_.personId == newMarker.personId)
        .map { existingMarker =>
          ...
          if (newMarker.round > existingMarker.round) {
            ...
            if (newMarker.distance < existingMarker.distance) {
              ...
              existingMarker.copy(sourceDirections =
                existingMarker.sourceDirections ++ newMarker.sourceDirections
              )
            }
          }
        }
  ...
  private def handleIncomingPerson(
      iteration: Long,
      cell: UrbanCell,
      person: Person,
      markerRound: Long
    ...
    cell.entrances
      ...
      .getOrElse(
        (putPerson(cell, person, markerRound), UrbanMetrics.empty)
      ) // no entrance in cell is the target of person
  }

  private def addVisitor(time: Double, cell: UrbanCell, entrance: Entrance, person: Person)(implicit
      config: UrbanConfig
  ): UrbanCell = {
    val newVisitor = Visitor(person, time + visitDuration(entrance))
    val newEntrances =
      cell.entrances.filterNot(_.targetId == entrance.targetId) :+ entrance.copy(visitors =
        entrance.visitors :+ newVisitor
      )
    cell.copy(entrances = newEntrances)
  }

  private def putPerson(cell: UrbanCell, person: Person, markerRound: Long)(implicit
      config: UrbanConfig
  ): CellContents = {
  ...
  private def visitDuration(entrance: Entrance)(implicit config: UrbanConfig): Double = {
    config.randomVisitTime(
      entrance.targetTypes.toSeq(config.random.nextInt(entrance.targetTypes.size))
    )
  }
}
