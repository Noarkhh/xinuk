...
final case class UrbanPlanCreator() extends PlanCreator[UrbanConfig] with LazyLogging {
  ...
  override def createPlans(
    ...
    if (config.pathCreation != "None") {
      ...
      move(
        ...
        cellState.signalMap,
        cellState.contents.asInstanceOf[UrbanCell],
        neighbourContents
      )
    }
  }
  ...
  private def move(
    ...
    if (timeState.personMovementIteration) {
      allMoves = allMoves ++ contents.occupants.map(person =>
        handlePerson(
          cellId,
          person,
          timeState.time,
          timeState.markerRound,
          contents.markers,
          neighbourContents
        )
      )
      allMoves = allMoves ++ contents.entrances.map(entrance =>
        handleEntrance(cellId, entrance, timeState.time, timeState.markerRound)
      )
    }
    allMoves = allMoves ++ contents.markers.map(marker =>
      handleMarker(marker, timeState.markerRound, neighbourContents)
    )

    allMoves
      .reduceOption[(Plans, UrbanMetrics)] {
  ...
  private def randomTargetByType(
  ...
  private def nearestTargetByType(cellId: GridCellId, targetType: TargetType)(implicit
      config: UrbanConfig
  ): Option[(String, TravelMode)] = {
    val targetInfo = config
      .targetTypeToTargets(targetType)
      .minBy(targetInfo => GridCellId.distance(cellId, targetInfo.center.gridId))
    Some(targetInfo.id, TravelMode.Travel)
  }

  private def chooseTarget(cellId: GridCellId, targetType: TargetType)(implicit
      config: UrbanConfig
  ): Option[(String, TravelMode)] = {
    targetType match {
  ...
  private def handleEntrance(
      cellId: GridCellId,
      entrance: Entrance,
      time: Double,
      markerRound: Long
  )(implicit config: UrbanConfig): (Plans, UrbanMetrics) = {
    val spawningPlans: (Plans, UrbanMetrics) =
      if (entrance.targetTypes.contains(TargetType.Residential)) {
        config
          .getTimeOfDay(time)
          .map { currentTimeOfDay =>
            val personSpawnProbability =
              config.getPersonSpawnProbability(currentTimeOfDay, entrance.population)
            if (config.random.nextDouble() <= personSpawnProbability) {
              ...
              val targetType = targetDistribution
              ...
              if (
                config.random.nextDouble() < config.personBehavior.restrictionFactors(targetType)
              ) {
                chooseTarget(cellId, targetType)
                  .map { case (target, travelMode) =>
    ...
    val returningPlans = entrance.visitors.map {
      case Visitor(person, returnTime) if time >= returnTime =>
        ...
        (
          Plans(
            None -> Plan(
              CreatePerson(returningPerson, markerRound),
              RemoveVisitor(entrance.targetId, person.id)
            )
          ),
          UrbanMetrics.empty
        )
  ...
  private val weightedDirections: Map[GridDirection, Map[GridDirection, Double]] = {
    ...
    GridDirection.values.map { direction =>
      direction -> Map(
        direction -> 1d,
        direction.clockwise -> (1d - step),
        direction.counterClockwise -> (1d - step - clockwiseBias)
      )
    }.toMap
  }
  ...
  private def chooseDirection(
      ...
      personId: String,
      markers: Seq[PersonMarker],
      allowedDirections: Set[Direction]
  )(implicit config: UrbanConfig): Option[Direction] = {
    val markerDistancePenalty: PartialFunction[Double, Double] = { case distance =>
      0.6 - distance * 0.1
    }
    ...
    if (weighted.isEmpty) {
      ...
      val markerSourceDirections = markers
        ...
        .flatMap(marker =>
          marker.sourceDirections.map(direction =>
            (direction, markerDistancePenalty(marker.distance))
          )
        )
        .filter { case (direction, _) => allowedDirections.contains(direction) }
        .toMap

      val finalScoring = weighted.keySet.map(d =>
        d -> (weighted.getOrElse(d, 0d) - markerSourceDirections.getOrElse(d, 0d))
      )
  ...
  private def handlePerson(
    ...
    val allowedDirections = neighbourContents
      ...
      .keySet // remove directions to unwalkable cells
    val preferredDirections = allowedDirections.filterNot(
      historicalDirections.contains
    ) // remove previously taken decisions
    val staticDirectionOpt = config.staticPaths(updatedPerson.target).get(cellId)

    val plans = staticDirectionOpt
      .map { staticDirection =>
        val chosenDirectionOpt =
          chooseDirection(staticDirection, updatedPerson.id, markers, preferredDirections)
        chosenDirectionOpt
          .map { chosenDirection =>
            movePlans(cellId, chosenDirection, updatedPerson, markerRound)
          }
          .getOrElse {
            val altChosenDirectionOpt =
              chooseDirection(staticDirection, updatedPerson.id, markers, allowedDirections)
            altChosenDirectionOpt
              .map { chosenDirection =>
                logger.warn(
                  s"Forced to repeat historic move from $cellId to ${updatedPerson.target}: $chosenDirection"
                )
                movePlans(cellId, chosenDirection, updatedPerson, markerRound)
              }
              .getOrElse {
  ...
  private def checkAndPurgeMarkers(cellId: GridCellId, contents: UrbanCell, markerRound: Long)(
    ...
    val metrics = contents.occupants
      .map { person =>
  ...
  override def finalize(worldShard: WorldShard)(implicit config: UrbanConfig): Unit = {
    if (config.pathCreation != "None") {
      val signal: Map[GridCellId, SignalMap] = worldShard.localCellIds
