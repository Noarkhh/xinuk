...
object Merge extends App {
          ...
          line.split(';') match {
            case Array(
              ...
              Metrics(
                ...
                aCount.toLong,
                fDeaths.toLong,
                fTotalEnergy.toDouble,
                fReproductions.toLong,
                fConsumed.toLong,
                fTotalLifespan.toLong,
                aTotalLifespan.toLong
              )
          }
  ...
  println(
    "iteration,foraminiferaCount,algaeCount,foraminiferaTotalEnergy,foraminiferaReproductionCount,foraminiferaAverageDeadLifespan,algaeAverageDeadLifespan"
  )
  var iteration = 1L
  while (iterators.exists(_.nonEmpty)) {
    val total = iterators
      .flatMap { iterator =>
        ...
        while (iterator.nonEmpty && iterator.head.iteration == iteration)
    ...
    val result = ParsedMetrics(
      ...
      total.algaeCount,
      if (total.foraminiferaDeaths != 0)
        total.foraminiferaTotalLifespan.toDouble / total.foraminiferaDeaths
      else 0,
      if (total.consumedAlgaeCount != 0)
        total.algaeTotalLifespan.toDouble / total.consumedAlgaeCount
      else 0,
