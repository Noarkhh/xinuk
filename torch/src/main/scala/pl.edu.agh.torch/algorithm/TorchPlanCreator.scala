...
final case class TorchPlanCreator() extends PlanCreator[TorchConfig] {
  ...
  override def createPlans(
    ...
    cellState.contents match {
      ...
      case person: Person =>
        (movePerson(iteration, person, cellState.signalMap, neighbourContents), TorchMetrics.person)
      case Exit => (Plans.empty, TorchMetrics.exit)
      case _    => (Plans.empty, TorchMetrics.empty)
    }
  }

  private def spreadFire(iteration: Long, neighbourContents: Map[Direction, CellContents])(implicit
      config: TorchConfig
  ): Plans = {
    if (iteration % config.fireSpreadingFrequency == 0) {
      val availableDirections: Seq[Direction] =
        neighbourContents.filterNot(_._2 == Obstacle).keys.toSeq
  ...
  private def movePerson(
      iteration: Long,
      person: Person,
      cellSignal: SignalMap,
      neighbourContents: Map[Direction, CellContents]
  )(implicit config: TorchConfig): Plans = {
    if (iteration % person.speed == 0) {
      val directions =
        Random
          .shuffle(cellSignal.filter { case (direction, _) =>
            neighbourContents.contains(direction) &&
            (neighbourContents(direction) == Empty || neighbourContents(direction) == Exit)
          }.toSeq)
          .sortBy(_._2)
          .reverse
          .map(_._1)
